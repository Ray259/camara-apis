...
export function RetrieveScoringDoc() {
  return applyDecorators(
    ...
    ApiResponse({
      status: 400,
      description: 'INVALID_ARGUMENT - Invalid request format',
    }),
    ApiResponse({
      status: 401,
    ...
    ApiResponse({
      status: 403,
      description: 'PERMISSION_DENIED - Insufficient permissions',
    }),
    ApiResponse({
      status: 404,
      description:
        'NOT_FOUND / IDENTIFIER_NOT_FOUND - Resource or phone not found',
    }),
    ApiResponse({
      status: 422,
      description:
        'MISSING_IDENTIFIER / UNNECESSARY_IDENTIFIER / SERVICE_NOT_APPLICABLE / CUSTOMER_INSIGHTS.* errors',
    }),
    ApiResponse({
      status: 429,
      description: 'TOO_MANY_REQUESTS - Rate limit exceeded',
    }),
  );
}
